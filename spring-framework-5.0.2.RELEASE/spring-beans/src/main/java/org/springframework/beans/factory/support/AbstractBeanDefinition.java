...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	/**
	 * 当前的bean创建 必须要依赖哪些bean先被创建
	 */
	@Nullable
	private String[] dependsOn;

	/**
	 * 设置为false的时候，这样容器在自动装配对象的时候，会不考虑当前的bean（他不会被看作为其他bean依赖的bean，但是他依赖其他的bean是能够被自动装配进来的）
	 */
	private boolean autowireCandidate = true;

	/**
	 * 当发现自动装配的时候，假如某个bean会发现多个，那么标注了primary 为true的首先被注入
	 */
