...
public class AsyncProcessor {
	...
	static {
		...
		try {
			...
			Runtime.getRuntime().addShutdownHook(new Thread(new Runnable() {
				...
				@Override
				public void run() {
					System.out.println("AsyncProcessor shutting down.");
					...
					try {
						...
						if (!executor.awaitTermination(3, TimeUnit.SECONDS)) {
							System.out.println("AsyncProcessor shutdown immediately due to wait timeout.");
							executor.shutdownNow();
						}
					} catch (InterruptedException e) {
						System.out.println("AsyncProcessor shutdown interrupted.");
						executor.shutdownNow();
					}

					System.out.println("AsyncProcessor shutdown complete.");
				}
			}));
		} catch (Exception e) {
			System.out.println("AsyncProcessor init error." + e);
	...
	public static boolean executeTask(Runnable task) {
		...
		try {
			...
			System.out.println("Task executing was rejected."+ e);
			return false;
		}
